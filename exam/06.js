...
let sortedByScores = students.slice().sort((a, b) => (a.scores > b.scores ? -1 : 1))
...
for (let i = 0; i < students.length; i++) {
	if (bestStudent.scores < students[i].scores) {
...
let scoresSum = students.reduce((acc, current) => acc + current.scores, 0)
...
const addFriends = students => {
	return students.map(student => ({
		...student,
		friends: students.filter(el => el.name !== student.name).map(el => el.name),
	}))
}
